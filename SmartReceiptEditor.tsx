...
export default function SmartReceiptEditor() {
    const {
        ...
        offlineReceiptLogo: state.offlineReceiptLogo,
        setOfflineReceiptLogo: state.setOfflineReceiptLogo,
        offlineReceiptPromo: state.offlineReceiptPromo,
        setOfflineReceiptPromo: state.setOfflineReceiptPromo,
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, stateSetter: (d: string | null) => void) => {
        ...
        reader.onloadend = () => {
            ...
            stateSetter(dataUrl);
            const storageKey = stateSetter === setOfflineReceiptLogo ? 'logo' : 'promo';
            showNotification(`อัปโหลดและบันทึกรูปสำหรับ "${storageKey}" เรียบร้อยแล้ว`, 'success');
        };
        reader.readAsDataURL(file);
    };

    const handleImageDelete = async (stateSetter: (d: string | null) => void) => {
        const keyName = stateSetter === setOfflineReceiptLogo ? 'logo' : 'promo';
                    ...
                    <div className="upload-control">
                             ...
                             {offlineReceiptLogo ? (
                                <>
                                    <img src={offlineReceiptLogo} alt="Offline Logo Preview" />
                                     {isAdminMode && <button className="delete-upload-btn" onClick={() => handleImageDelete(setOfflineReceiptLogo)}>&times;</button>}
                                </>
                        ...
                        {isAdminMode && (
                            <label className="action-button">
                                <UploadIcon />
                                 <span>อัปโหลด</span>
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, setOfflineReceiptLogo)} />
                            </label>
                         )}
                    </div>
                </div>
                            ...
                            {offlineReceiptPromo ? (
                                <>
                                    <img src={offlineReceiptPromo} alt="Offline Promo Preview" />
                                     {isAdminMode && <button className="delete-upload-btn" onClick={() => handleImageDelete(setOfflineReceiptPromo)}>&times;</button>}
                                </>
                            ) : <p className="text-secondary" style={{fontSize: '0.9rem'}}>ไม่มีรูป</p>}
                         </div>
                        {isAdminMode && (
                            <label className="action-button">
                                <UploadIcon />
                                 <span>อัปโหลด</span>
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, setOfflineReceiptPromo)} />
                            </label>
                         )}
                    </div>
                </div>
