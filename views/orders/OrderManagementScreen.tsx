...
const OrderManagementScreen: React.FC<OrderManagementScreenProps> = () => {
    ...
    const todaysTotalSales = useMemo(() => {
        ...
        return completedOrders
            .filter(order => getYYYYMMDD(new Date(order.timestamp)) === todayStr && order.total > 0)
                ...
                {activeKdsTab === 'bills' && (
                    <div className="completed-bills-list-container">
                        ...
                        <footer className="kds-summary-footer">
                            <span>ยอดขายรวมวันนี้ (ไม่รวมบิลยกเลิก)</span>
                            <span className="footer-total">฿{formatCurrency(todaysTotalSales)}</span>
                        </footer>
                    </div>
                )}
                 {activeKdsTab === 'shift' && 
